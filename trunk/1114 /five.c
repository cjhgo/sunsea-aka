...
void get_player_win_table_max(void)
{
	...
	for( i = 0; i < NUM; i++)
	{
		for(j = 0; j < NUM; j++)
		{
			if(pos_flag[j][i] == 0)
			{
				...
				player_win_table[j][i] = left_score + right_score + up_score + down_score + left_up_score + right_up_score + 
	...
	for(i = 0; i < NUM; i++)
	{
		for(j = 0; j < NUM; j++)
		{
			if(player_win_table[j][i] > player_max)
			{
				player_max_x = j;
				player_max_y = i;
...
void get_computer_win_table_max(void)
{
	...
	//set computer_win_table 0
	for(i = 0; i < NUM; i++)
	{
		for(j = 0; j < NUM; j++)
		{
			computer_win_table[j][i] = 0;
		}
	}
	...
	for(i = 0; i < NUM; i++)
	{
		for(j = 0; j < NUM; j++)
		{
			if(pos_flag[j][i] == 0)
			{
				...
				computer_win_table[j][i] = left_score + right_score + up_score + down_score + left_up_score + right_up_score + 
					left_down_score + right_down_score;
			}
		}
	}
	...
	for(i = 0; i < NUM; i++)
	{
		for(j = 0; j < NUM; j++)
		{
			if(computer_win_table[j][i] > computer_max)
			{
				computer_max_x = j;
				computer_max_y = i;
