...
void get_player_win_table_max(int player_chess)
{
	int i = 0, j = 0, k, p, m, n;
	...
	for(; i < NUM; i++)
	{
		for(; j < NUM; j++)
		{
			if(player_chess[i][j] == 0)
			{
				...
				for(; k >= 0; k--)
				{
					if(player_chess[i][k] == 1)
						n++;
					else
					{
						if(player_chess[i][k] == 2)
							m = 0;
						else if(player_chess[i][k] == 0)
							m = 1;
				...
				for(; k >= 0; k--)
				{
					if(player_chess[k][j] == 1)
						n++;
					else
					{
						if(player_chess[k][j] == 2)
							m = 0;
						else if(player_chess[k][j] == 0)
							m = 1;
						break;
					}
				}
				...
				for(; k >= 0, p >=0; k--, p--)
				{
					if(player_chess[k][p] == 1)
						n++;
					else
					{
						if(player_chess[k][p] == 2)
							m = 0;
						else if(player_chess[k][p] == 0)
							m = 1;
						break;
					}
				}
				...
				player_win_table[i][j] = left_score + right_score + up_score + down_score + left_up_score + right_up_score + left_down_score + right_down_score;
			}
		}
	}
...
void get_computer_win_table(int computer_chess)
{
	int i = 0, j = 0, k, p, m, n;
	int left_score, right_score, up_score, down_score, left_up_score, right_up_score, left_down_score, right_down_score;
	...
	for(; i < NUM; i++)
	{
		for(; j < NUM; j++)
		{
			if(computer_chess[i][j] == 0)
			{
				...
				for(; k >= 0; k--)
				{
					if(computer_chess[i][k] == 2)
						n++;
					else
					{
						if(computer_chess[i][k] == 1)
							m = 0;
						else if(computer_chess[i][k] == 0)
							m = 1;
				...
				for(; k >= 0; k--)
				{
					if(computer_chess[k][j] == 2)
						n++;
					else
					{
						if(computer_chess[k][j] == 1)
							m = 0;
						else if(computer_chess[k][j] == 0)
							m = 1;
						break;
					}
				}
				...
				for(; k >= 0, p >=0; k--, p--)
				{
					if(computer_chess[k][p] == 2)
						n++;
					else
					{
						if(computer_chess[k][p] == 1)
							m = 0;
						else if(computer_chess[k][p] == 0)
							m = 1;
						break;
					}
				}
...
//	fbmem = mmap(0, w*h*sizeof(short), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);		//use 16
	...
	fb_num(chess_num++, 8 * SPACE + X, 8 * SPACE + Y, 0x0);
