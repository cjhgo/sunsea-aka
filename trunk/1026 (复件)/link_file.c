...
//处理网址结束位置
void judge_end(char **p, int *count)
{
	int i = 0;
	PNODE new;
...
void get_every_link(PNODE temp, int *pos, int *count, int *ret_read_file)
{
	int i = 0;
	while(0 != *ret_read_file)
	{
		...
		while('\0' != buf[i])
		{
			*count = 0;
			...
			link(&buf[i], count);					//得到合法网址，并存到全局变量link_temp中
...
int main(int argc, char* argv[])
{
	...
	get_every_link(temp, &pos, &count, &ret_read_file);			//循环读文件，得到每一个合法的网址
